...
namespace aqq
{
    public partial class SkipPage : TabbedPage
    {
        ...
        async void OnImage5Tapped(object sender, EventArgs args)
        {
            ...
            if (Application.Current.Properties.ContainsKey("Status"))
            {
                ...
                if (st.Equals("Volunteer") || st.Equals("Mentor"))
                {
                    ...
                    if (Application.Current.Properties.ContainsKey("Name"))
                    {
                       // await DisplayAlert("Alert", "This function only providing for learner", "OK");
                       await Navigation.PushAsync(new TeachPage());

                    }
                    else
                    {
                        await DisplayAlert("Alert", "Please go to 'Me' Button below to complete your personal profile", "OK");
                ...
                if (st.Equals("Learner"))
                {
                    if (Application.Current.Properties.ContainsKey("Name"))
                    {
                       // var skill = Application.Current.Properties["Skill"] as string;
						//await Navigation.PushAsync(new NameListPage(skill));
		...
		async void OnImage7Tapped(object sender, EventArgs args)
		{
			...
			else
			{
				...
				if (result == true)
				{
					Application.Current.Properties["Click"] = "course";
					await Navigation.PushAsync(new Login());
				}
			}

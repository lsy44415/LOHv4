...
namespace aqq
{
    public partial class FriendListPage : ContentPage
    {
        ...
        async Task Notifaction(string email){
			...
			if (friend.Count > 0)
			{
				for (int i = 0; i < friend.Count; i++)
				{
					...
					var result = await DisplayAlert("New Notifaction", st, "Accept", "Reject");
                    ...
                    else
                    {
						...
						await fri.UpdateTaskAsync(updatefriend);
						await DisplayAlert("New Notifaction", "This person could not send your message and email from now on.", "OK");
						listbyoth.ItemsSource = await fri.GetfollowyouPeersAsync(email);
                    }
		...
		{
			var result = await DisplayAlert("Alert", "Unfollow this person ? ", "Yes", "Cancel");
            if (result == true)
            {
                var email = Application.Current.Properties["Email"] as string;
		...
		{
			var result = await DisplayAlert("Alert", "Remove this follower? ", "Yes", "Cancel");
			if (result == true)
			{
				var email = Application.Current.Properties["Email"] as string;
				var menuItem = sender as MenuItem;
